 ...
 */
export default class Map3 extends Cartogram {
  ...
  join(countries, joinData) {
    countries.features.forEach(feature => {
      const countryData = joinData
        .filter(row => row.iso3code === this.getISO3(feature));
  ...
  countryFill(d) {
    if (d && d.properties) {
      const { joined } = d.properties; 
      if (joined) {
        const matchingYears = joined.filter(({ year }) => year === this.filterState.year);
