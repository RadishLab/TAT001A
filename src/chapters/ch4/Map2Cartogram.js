...
export default class Map2Cartogram extends Cartogram {
    ...
    countries.features.forEach(feature => {
      ...
      if (countryData.length > 0) {
        ...
        this.filterColumns.forEach(column => {
          const value = feature.properties.joined[column.symbol];
          feature.properties.joined[column.symbol] = value && value !== 'No';
        });
      }
    });
    return countries;
  ...
  updateFilters(selectedFilter) {
    super.updateFilters(selectedFilter);

    this.addSymbolPolygons(this.getMatchingFilter().symbol);
  }
